...
## CW's version 3 of original palaeo-emulator, Which is the same as version 2 but reads in all available proxy data (so Dome C, 722, 846, 982, 1143 and 1090) and 
## creates figures showing all 2.58 Ma from emulator (with first 800 kya as a different colour), all 2.58 Ma proxy data (for 722, 846 and 982) and 800 kya proxy data (for 
## Dome C, 1143 and 1090)
## Working (as of 5/1/23)
...
#for (i in 1:length(timeslices)){
  #png(file=paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output/Plots/Other\\SAT\\Mp_em_SAT_rcp8.5_upd_",i_order[i],"_SLR",nam_SLR[i],"m_",nam_timeslice[i],"kyrAP.png", sep=""),width=1000,height=600)
...
#for (i in 1:length(timeslices_tdab)){
  #png(file=paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output/Plots/Other\\SAT\\Mp_tdab_SAT_",i_order_tdab[i],"_SLR",nam_SLR_tdab[i],"m_",nam_timeslice_tdab[i],"kyrBP.png", sep=""),width=1000,height=600)
  #image.plot(lons, lats, model_output_modice_tdab_big_mod[,c(73:1),timeslices_tdab[i]], zlim=c(-20,20), col=col_bwr, xlab="", ylab="", main=paste("tdab SAT anomaly at ",nam_timeslice_tdab[i]," kyr BP (SLR ",nam_SLR_tdab[i]," m)", sep=""),
  #           axis.args=list(at=seq(-20, 20, 4),labels=labels_plot,cex.axis=1), 
  #           legend.args=list(text='', side=4, font=4, line=2.5, cex=0.8))
  #map(add=T, interior=F, col="black")
  #dev.off()
...
# All 2.5 Ma proxy data (for 722, 846 and 982), 800 kya proxy data (for 1143, 1090 and Dome C)

print("Reading 2.5 Ma data")

# Read in SST data at ODP-722 for 2.5-0 Ma BP [Herbert et al 2010]

source("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Emulator/2015_Bristol_5D_v001/Data/2018-08-01 Final report/Import_SST_ODP722_Herbertetal10_2Ma_BP_cjrw.R")

# CW: where [,9] = years (ka), [,13] = SST absolutes (Pr), [,14] = SST absolutes (Muller), [,17] = SST anomalies (uncorrected) and [,18] = SST anomalies (corrected for plate motion)
# However, absolutes are very very similar, and anomalies are indistinguishable

data_interp = approx(data_SST_ODP722_Herbertetal10_2Ma_BP[,9],data_SST_ODP722_Herbertetal10_2Ma_BP[,14], xout=rev(times)) # Interpolate proxy data to 1 kyr resolution for calculating correlation coefficient
data_SST_ab_ODP722_Herbertetal10_2Ma_BP_interp = array(c(data_interp[1]$x,data_interp[2]$y),c(length(rev(times)),2))

data_interp = approx(data_SST_ODP722_Herbertetal10_2Ma_BP[,9],data_SST_ODP722_Herbertetal10_2Ma_BP[,17], xout=rev(times)) # Interpolate proxy data to 1 kyr resolution for calculating correlation coefficient
data_SST_anom_ODP722_Herbertetal10_2Ma_BP_interp = array(c(data_interp[1]$x,data_interp[2]$y),c(length(rev(times)),2))

# plot(data_SST_ODP722_Herbertetal10_2Ma_BP[,9],data_SST_ODP722_Herbertetal10_2Ma_BP[,13], type="l")
# plot(data_SST_ODP722_Herbertetal10_2Ma_BP_interp[,1],data_SST_ODP722_Herbertetal10_2Ma_BP_interp[,2], type="l")

###

# Read in SST data at ODP-846 for 2.5-0 Ma BP [Liu & Herbert 2004]

source("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Emulator/2015_Bristol_5D_v001/Data/2018-08-01 Final report/Import_SST_ODP846_LiuHerbert04_2Ma_BP_cjrw.R")

# CW: where [,7] = years (ka), [,10] = SST absolutes (P&W), [,11] = SST absolutes (Muller), [,13] = SST anomalies (uncorrected) and [,14] = SST anomalies (corrected for plate motion)
# However, absolutes are very very similar, and anomalies are indistinguishable

data_interp = approx(data_SST_ODP846_LiuHerbert04_2Ma_BP[,7],data_SST_ODP846_LiuHerbert04_2Ma_BP[,11], xout=rev(times)) # Interpolate proxy data to 1 kyr resolution for calculating correlation coefficient
data_SST_ab_ODP846_LiuHerbert04_2Ma_BP_interp = array(c(data_interp[1]$x,data_interp[2]$y),c(length(rev(times)),2))

data_interp = approx(data_SST_ODP846_LiuHerbert04_2Ma_BP[,7],data_SST_ODP846_LiuHerbert04_2Ma_BP[,13], xout=rev(times)) # Interpolate proxy data to 1 kyr resolution for calculating correlation coefficient
data_SST_anom_ODP846_LiuHerbert04_2Ma_BP_interp = array(c(data_interp[1]$x,data_interp[2]$y),c(length(rev(times)),2))

# plot(data_SST_ODP846_LiuHerbert04_2Ma_BP[,9],data_SST_ODP846_LiuHerbert04_2Ma_BP[,13], type="l")
# plot(data_SST_ODP846_LiuHerbert04_2Ma_BP_interp[,1],data_SST_ODP846_LiuHerbert04_2Ma_BP_interp[,2], type="l")

###

# Read in SST data at ODP-982 for 2.5-0 Ma BP [Lawrence et al 2009]

source("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Emulator/2015_Bristol_5D_v001/Data/2018-08-01 Final report/Import_SST_ODP982_Lawrenceetal09_2Ma_BP_cjrw2.R")

# CW: where [,9] = years (ka), [,13] = SST absolutes (P&W), [,14] = SST absolutes (Muller), [,16] = SST anomalies (uncorrected) and [,17] = SST anomalies (corrected for plate motion)
# However, absolutes are very very similar, and anomalies are indistinguishable

data_interp = approx(data_SST_ODP982_Lawrenceetal09_2Ma_BP[,9],data_SST_ODP982_Lawrenceetal09_2Ma_BP[,14], xout=rev(times)) # Interpolate proxy data to 1 kyr resolution for calculating correlation coefficient
data_SST_ab_ODP982_Lawrenceetal09_2Ma_BP_interp = array(c(data_interp[1]$x,data_interp[2]$y),c(length(rev(times)),2))

data_interp = approx(data_SST_ODP982_Lawrenceetal09_2Ma_BP[,9],data_SST_ODP982_Lawrenceetal09_2Ma_BP[,16], xout=rev(times)) # Interpolate proxy data to 1 kyr resolution for calculating correlation coefficient
data_SST_anom_ODP982_Lawrenceetal09_2Ma_BP_interp = array(c(data_interp[1]$x,data_interp[2]$y),c(length(rev(times)),2))

# plot(data_SST_ODP982_Lawrenceetal09_2Ma_BP[,9],data_SST_ODP982_Lawrenceetal09_2Ma_BP[,13], type="l")
# plot(data_SST_ODP982_Lawrenceetal09_2Ma_BP_interp[,1],data_SST_ODP982_Lawrenceetal09_2Ma_BP_interp[,2], type="l")

print("Reading 800 kya data")
...
data_interp = approx(data_SST_ODP1143_Lietal11_800kyr_BP[,2],data_SST_ODP1143_Lietal11_800kyr_BP[,3], xout=rev(times2)) # Interpolate proxy data to 1 kyr resolution for calculating correlation coefficient
data_SST_ODP1143_Lietal11_800kyr_BP_interp = array(c(data_interp[1]$x,data_interp[2]$y),c(length(rev(times2)),2))
...
data_interp = approx(data_SST_ODP1090_MartinezGarciaetal10_800kyr_BP[,2],data_SST_ODP1090_MartinezGarciaetal10_800kyr_BP[,3], xout=rev(times2)) # Interpolate proxy data to 1 kyr resolution for calculating correlation coefficient
data_SST_ODP1090_MartinezGarciaetal10_800kyr_BP_interp = array(c(data_interp[1]$x,data_interp[2]$y),c(length(rev(times2)),2))
...
# Update 2023: this (and sane in previous versions) is actually wrong, because absolutes = anomaly + PI, not anomaly + anomaly.  Don't know what PI is in paper, but using 
# modern value of -50.3
...
data_interp = approx(data_D_Jouzeletal07_800kyr_BP[,2],data_D_Jouzeletal07_800kyr_BP[,3], xout=rev(times2)) # Interpolate proxy data to 1 kyr resolution for calculating correlation coefficient
data_D_Jouzeletal07_800kyr_BP_interp = array(c(data_interp[1]$x,data_interp[2]$y),c(length(rev(times2)),2))

data_D_Jouzeletal07_800kyr_BP_interp_ab = array(c(data_interp[1]$x,data_interp[2]$y),c(length(rev(times2)),2)) # Create another array identical to above
...
data_D_Jouzeletal07_800kyr_BP_interp_ab[,2] = data_D_Jouzeletal07_800kyr_BP_interp_ab[,2] + -50.3
...
# Calculate anomaly between proxy data and PI in proxy data (only for 1143 and 1090, because others (going back to 2.5 Ma) automatically give anomalies,
# as does Dome C)
...
## Print out 2.58 Ma SST data, absolutes and anomalies

print("Saving 2.5 Ma SST data absolute values")

ncol = 2

data_to_save = array(c(times, round(data_SST_ab_ODP722_Herbertetal10_2Ma_BP_interp[,2], digits = 5)),c(length(times),ncol)) # CW: Need to take 2nd column i.e. temperature [,2]
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Interpolated proxy SST data/proxy_SST_ab_ODP722_interp_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)

data_to_save = array(c(times, round(data_SST_ab_ODP846_LiuHerbert04_2Ma_BP_interp[,2], digits = 5)),c(length(times),ncol)) # CW: Need to take 2nd column i.e. temperature [,2]
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Interpolated proxy SST data/proxy_SST_ab_ODP846_interp_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)

data_to_save = array(c(times, round(data_SST_ab_ODP982_Lawrenceetal09_2Ma_BP_interp[,2], digits = 5)),c(length(times),ncol)) # CW: Need to take 2nd column i.e. temperature [,2]
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Interpolated proxy SST data/proxy_SST_ab_ODP982_interp_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)

print("Saving 2.5 Ma SST data anomalies")

data_to_save = array(c(times, round(data_SST_anom_ODP722_Herbertetal10_2Ma_BP_interp[,2], digits = 5)),c(length(times),ncol)) # CW: Need to take 2nd column i.e. temperature [,2]
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Interpolated proxy SST data/proxy_SST_anom_ODP722_interp_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)

data_to_save = array(c(times, round(data_SST_anom_ODP846_LiuHerbert04_2Ma_BP_interp[,2], digits = 5)),c(length(times),ncol)) # CW: Need to take 2nd column i.e. temperature [,2]
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Interpolated proxy SST data/proxy_SST_anom_ODP846_interp_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)

data_to_save = array(c(times, round(data_SST_anom_ODP982_Lawrenceetal09_2Ma_BP_interp[,2], digits = 5)),c(length(times),ncol)) # CW: Need to take 2nd column i.e. temperature [,2]
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Interpolated proxy SST data/proxy_SST_anom_ODP982_interp_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)
...
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Data/Em_output_data_temp_absolutes_ODP982_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)

data_to_save = array(c(times, round(my_tim_means_ODP722_paleodata_e, digits = 5)),c(length(times),ncol))
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Data/Em_output_data_temp_absolutes_ODP722_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)

data_to_save = array(c(times, round(my_tim_means_ODP1143_paleodata_e, digits = 5)),c(length(times),ncol))
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Data/Em_output_data_temp_absolutes_ODP1143_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)

data_to_save = array(c(times, round(my_tim_means_ODP846_paleodata_e, digits = 5)),c(length(times),ncol))
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Data/Em_output_data_temp_absolutes_ODP846_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)

data_to_save = array(c(times, round(my_tim_means_ODP1090_paleodata_e, digits = 5)),c(length(times),ncol))
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Data/Em_output_data_temp_absolutes_ODP1090_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)

data_to_save = array(c(times, round(my_tim_means_DomeC_paleodata_e, digits = 5)),c(length(times),ncol))
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Data/Em_output_data_temp_absolutes_DomeC_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)
...
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Data/Em_output_data_temp_ODP982_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)

data_to_save = array(c(times, round(my_tim_anom_ODP722_paleodata_e, digits = 5), round(my_var_anom_ODP982_paleodata_e, digits = 5)),c(length(times),ncol))
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC", "Uncertainty_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Data/Em_output_data_temp_ODP722_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)

data_to_save = array(c(times, round(my_tim_anom_ODP1143_paleodata_e, digits = 5), round(my_var_anom_ODP982_paleodata_e, digits = 5)),c(length(times),ncol))
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC", "Uncertainty_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Data/Em_output_data_temp_ODP1143_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)

data_to_save = array(c(times, round(my_tim_anom_ODP846_paleodata_e, digits = 5), round(my_var_anom_ODP982_paleodata_e, digits = 5)),c(length(times),ncol))
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC", "Uncertainty_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Data/Em_output_data_temp_ODP846_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)

data_to_save = array(c(times, round(my_tim_anom_ODP1090_paleodata_e, digits = 5), round(my_var_anom_ODP982_paleodata_e, digits = 5)),c(length(times),ncol))
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC", "Uncertainty_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Data/Em_output_data_temp_ODP1090_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)
...
colnames(data_to_save) = c("Time_kyrAP", "Temp_degC", "Uncertainty_degC")
file_nam = paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Data/Em_output_data_temp_DomeC_natural_2.58_to_0MyrBP.txt", sep="")
write.table(data_to_save, file = file_nam, append = FALSE, sep = " ", col.names = TRUE, row.names = FALSE, quote = FALSE)
...
# Save figures.  Need to do this twice, the first time with the same y-axis and the second time with different y-axes
...
# FIGURE 5 - Plot emulated temperature and proxy data ---------------------------------------------------------

# Calculate error bars for mean annual temperature anomaly at sites for -800-0 kyr AP

my_tim_anom_ODP982_paleodata_e_lower_1sd = my_tim_anom_ODP982_paleodata_e-sqrt(my_var_anom_ODP982_paleodata_e)
my_tim_anom_ODP982_paleodata_e_upper_1sd = my_tim_anom_ODP982_paleodata_e+sqrt(my_var_anom_ODP982_paleodata_e)
  
my_tim_anom_ODP722_paleodata_e_lower_1sd = my_tim_anom_ODP722_paleodata_e-sqrt(my_var_anom_ODP722_paleodata_e)
my_tim_anom_ODP722_paleodata_e_upper_1sd = my_tim_anom_ODP722_paleodata_e+sqrt(my_var_anom_ODP722_paleodata_e)
  
my_tim_anom_ODP1143_paleodata_e_lower_1sd = my_tim_anom_ODP1143_paleodata_e-sqrt(my_var_anom_ODP1143_paleodata_e)
my_tim_anom_ODP1143_paleodata_e_upper_1sd = my_tim_anom_ODP1143_paleodata_e+sqrt(my_var_anom_ODP1143_paleodata_e)
  
my_tim_anom_ODP846_paleodata_e_lower_1sd = my_tim_anom_ODP846_paleodata_e-sqrt(my_var_anom_ODP846_paleodata_e)
my_tim_anom_ODP846_paleodata_e_upper_1sd = my_tim_anom_ODP846_paleodata_e+sqrt(my_var_anom_ODP846_paleodata_e)
  
my_tim_anom_ODP1090_paleodata_e_lower_1sd = my_tim_anom_ODP1090_paleodata_e-sqrt(my_var_anom_ODP1090_paleodata_e)
my_tim_anom_ODP1090_paleodata_e_upper_1sd=my_tim_anom_ODP1090_paleodata_e+sqrt(my_var_anom_ODP1090_paleodata_e)
  
my_tim_anom_DomeC_paleodata_e_lower_1sd = my_tim_anom_DomeC_paleodata_e-sqrt(my_var_anom_DomeC_paleodata_e)
my_tim_anom_DomeC_paleodata_e_upper_1sd = my_tim_anom_DomeC_paleodata_e+sqrt(my_var_anom_DomeC_paleodata_e)
...
### Plot evolution of mean annual temperature anomaly at Dome C for 2.58-0 Ma AP

# Extract first 800 kya (i.e. last 800 values from timeseries)

my_tim_anom_DomeC_paleodata_e_800=my_tim_anom_DomeC_paleodata_e[1781:2580,]

# Plot, with first 800 kya shown in a different colour

png(file=paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Plots/SAT_DomeC_2MaBP_sameyaxis.png", sep=""),width=3000,height=850,res=300)
par(fig=c(lb,rb,(bb),(bb+ht)), mar=c(2,4.5,0.5,3), pty="m", mgp=c(2,0.5,0), las=1, lwd=lns)
plot(-(times), my_tim_anom_DomeC_paleodata_e,type="l", cex=lns, col=col_emul, xlab="", ylab="", xlim=xlimits, ylim=ylimits, xaxt="n", yaxt="n", xaxs="i", yaxs="i", axes=FALSE)

lines(xlimits,c(0,0), lty=3, lwd=lns, col="grey49")
polygon(c(-(times), rev(-(times))), c(my_tim_anom_DomeC_paleodata_e_lower_1sd,rev(my_tim_anom_DomeC_paleodata_e_upper_1sd)), col = rgb(col_shade_lightblue[1],col_shade_lightblue[2],col_shade_lightblue[3],col_shade_lightblue[4]), border = NA)
lines(-(times), my_tim_anom_DomeC_paleodata_e, lwd=lns, col=col_emul)
lines(-(times2), my_tim_anom_DomeC_paleodata_e_800, lwd=lns, col=col_emul_800)

lines(-(data_D_Jouzeletal07_800kyr_BP_interp[,1]),data_D_Jouzeletal07_800kyr_BP_interp[,2], lty=2, lwd=lns, col=col_proxy)
...
legend((xlimits[2]-850), (((ylimits[2]-ylimits[1])*0.99)+ylimits[1]), c("Proxy record","Emulated data (800 kya)","Emulated data (1780 kya)"), pch=c(NA), lty=c(2,1,1), lwd=lns, cex=fs3, pt.cex=pts, col=c(col_proxy,col_emul_800,col_emul), bty="n")
par(las=1)

dev.off()

### Plot evolution of mean annual temperature anomaly at ODP-982, for 2.58-0 Ma AP

# Extract first 800 kya (i.e. last 800 values from timeseries)

my_tim_anom_ODP982_paleodata_e_800=my_tim_anom_ODP982_paleodata_e[1781:2580,]

# Plot, with first 800 kya shown in a different colour

png(file=paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Plots/SST_anom_ODP982_2MaBP_sameyaxis.png", sep=""),width=3000,height=850,res=300)
par(fig=c(lb,rb,(bb),(bb+ht)), mar=c(2,4.5,0.5,3), pty="m", mgp=c(2,0.5,0), las=1, lwd=lns)
plot(-(times), my_tim_anom_ODP982_paleodata_e,type="l", cex=lns, col=col_emul, xlab="", ylab="", xlim=xlimits, ylim=ylimits, xaxt="n", yaxt="n", xaxs="i", yaxs="i", axes=FALSE)

lines(xlimits,c(0,0), lty=3, lwd=lns, col="grey49")
polygon(c(-(times), rev(-(times))), c(my_tim_anom_ODP982_paleodata_e_lower_1sd,rev(my_tim_anom_ODP982_paleodata_e_upper_1sd)), col = rgb(col_shade_lightblue[1],col_shade_lightblue[2],col_shade_lightblue[3],col_shade_lightblue[4]), border = NA)
lines(-(times), my_tim_anom_ODP982_paleodata_e, lwd=lns, col=col_emul)
lines(-(times2), my_tim_anom_ODP982_paleodata_e_800, lwd=lns, col=col_emul_800)

lines(-(data_SST_anom_ODP982_Lawrenceetal09_2Ma_BP_interp[,1]),data_SST_anom_ODP982_Lawrenceetal09_2Ma_BP_interp[,2], lty=2, lwd=lns, col=col_proxy)
...
### Plot evolution of mean annual temperature anomaly at ODP-722, for 2.58-0 Ma AP

# Extract first 800 kya (i.e. last 800 values from timeseries)

my_tim_anom_ODP722_paleodata_e_800=my_tim_anom_ODP722_paleodata_e[1781:2580,]

# Plot, with first 800 kya shown in a different colour

png(file=paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Plots/SST_anom_ODP722_2MaBP_sameyaxis.png", sep=""),width=3000,height=850,res=300)
par(fig=c(lb,rb,(bb),(bb+ht)), mar=c(2,4.5,0.5,3), pty="m", mgp=c(2,0.5,0), las=1, lwd=lns)
plot(-(times), my_tim_anom_ODP722_paleodata_e,type="l", cex=lns, col=col_emul, xlab="", ylab="", xlim=xlimits, ylim=ylimits, xaxt="n", yaxt="n", xaxs="i", yaxs="i", axes=FALSE)

lines(xlimits,c(0,0), lty=3, lwd=lns, col="grey49")
polygon(c(-(times), rev(-(times))), c(my_tim_anom_ODP722_paleodata_e_lower_1sd,rev(my_tim_anom_ODP722_paleodata_e_upper_1sd)), col = rgb(col_shade_lightblue[1],col_shade_lightblue[2],col_shade_lightblue[3],col_shade_lightblue[4]), border = NA)
lines(-(times), my_tim_anom_ODP722_paleodata_e, lwd=lns, col=col_emul)
lines(-(times2), my_tim_anom_ODP722_paleodata_e_800, lwd=lns, col=col_emul_800)

lines(-(data_SST_anom_ODP722_Herbertetal10_2Ma_BP_interp[,1]),data_SST_anom_ODP722_Herbertetal10_2Ma_BP_interp[,2], lty=2, lwd=lns, col=col_proxy)
...
### Plot evolution of mean annual temperature anomaly at ODP-1143, for 2.58-0 Ma AP

# Extract first 800 kya (i.e. last 800 values from timeseries)

my_tim_anom_ODP1143_paleodata_e_800=my_tim_anom_ODP1143_paleodata_e[1781:2580,]

# Plot, with first 800 kya shown in a different colour

png(file=paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Plots/SST_anom_ODP1143_2MaBP_sameyaxis.png", sep=""),width=3000,height=850,res=300)
par(fig=c(lb,rb,(bb),(bb+ht)), mar=c(2,4.5,0.5,3), pty="m", mgp=c(2,0.5,0), las=1, lwd=lns)
plot(-(times), my_tim_anom_ODP1143_paleodata_e,type="l", cex=lns, col=col_emul, xlab="", ylab="", xlim=xlimits, ylim=ylimits, xaxt="n", yaxt="n", xaxs="i", yaxs="i", axes=FALSE)

lines(xlimits,c(0,0), lty=3, lwd=lns, col="grey49")
polygon(c(-(times), rev(-(times))), c(my_tim_anom_ODP1143_paleodata_e_lower_1sd,rev(my_tim_anom_ODP1143_paleodata_e_upper_1sd)), col = rgb(col_shade_lightblue[1],col_shade_lightblue[2],col_shade_lightblue[3],col_shade_lightblue[4]), border = NA)
lines(-(times), my_tim_anom_ODP1143_paleodata_e, lwd=lns, col=col_emul)
lines(-(times2), my_tim_anom_ODP1143_paleodata_e_800, lwd=lns, col=col_emul_800)

lines(-(data_SST_anom_ODP1143_Lietal11_800kyr_BP_interp[,1]),data_SST_anom_ODP1143_Lietal11_800kyr_BP_interp[,3], lty=2, lwd=lns, col=col_proxy)
...
### Plot evolution of mean annual temperature anomaly at ODP-846, for 2.58-0 Ma AP

# Extract first 800 kya (i.e. last 800 values from timeseries)

my_tim_anom_ODP846_paleodata_e_800=my_tim_anom_ODP846_paleodata_e[1781:2580,]

# Plot, with first 800 kya shown in a different colour

png(file=paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Plots/SST_anom_ODP846_2MaBP_sameyaxis.png", sep=""),width=3000,height=850,res=300)
par(fig=c(lb,rb,(bb),(bb+ht)), mar=c(2,4.5,0.5,3), pty="m", mgp=c(2,0.5,0), las=1, lwd=lns)
plot(-(times), my_tim_anom_ODP846_paleodata_e,type="l", cex=lns, col=col_emul, xlab="", ylab="", xlim=xlimits, ylim=ylimits, xaxt="n", yaxt="n", xaxs="i", yaxs="i", axes=FALSE)

lines(xlimits,c(0,0), lty=3, lwd=lns, col="grey49")
polygon(c(-(times), rev(-(times))), c(my_tim_anom_ODP846_paleodata_e_lower_1sd,rev(my_tim_anom_ODP846_paleodata_e_upper_1sd)), col = rgb(col_shade_lightblue[1],col_shade_lightblue[2],col_shade_lightblue[3],col_shade_lightblue[4]), border = NA)
lines(-(times), my_tim_anom_ODP846_paleodata_e, lwd=lns, col=col_emul)
lines(-(times2), my_tim_anom_ODP846_paleodata_e_800, lwd=lns, col=col_emul_800)

lines(-(data_SST_anom_ODP846_LiuHerbert04_2Ma_BP_interp[,1]),data_SST_anom_ODP846_LiuHerbert04_2Ma_BP_interp[,2], lty=2, lwd=lns, col=col_proxy)
...
### Plot evolution of mean annual temperature anomaly at ODP-1090, for 2.58-0 Ma AP

# Extract first 800 kya (i.e. last 800 values from timeseries)

my_tim_anom_ODP1090_paleodata_e_800=my_tim_anom_ODP1090_paleodata_e[1781:2580,]

# Plot, with first 800 kya shown in a different colour

png(file=paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Plots/SST_anom_ODP1090_2MaBP_sameyaxis.png", sep=""),width=3000,height=850,res=300)
par(fig=c(lb,rb,(bb),(bb+ht)), mar=c(2,4.5,0.5,3), pty="m", mgp=c(2,0.5,0), las=1, lwd=lns)
plot(-(times), my_tim_anom_ODP1090_paleodata_e,type="l", cex=lns, col=col_emul, xlab="", ylab="", xlim=xlimits, ylim=ylimits, xaxt="n", yaxt="n", xaxs="i", yaxs="i", axes=FALSE)

lines(xlimits,c(0,0), lty=3, lwd=lns, col="grey49")
polygon(c(-(times), rev(-(times))), c(my_tim_anom_ODP1090_paleodata_e_lower_1sd,rev(my_tim_anom_ODP1090_paleodata_e_upper_1sd)), col = rgb(col_shade_lightblue[1],col_shade_lightblue[2],col_shade_lightblue[3],col_shade_lightblue[4]), border = NA)
lines(-(times), my_tim_anom_ODP1090_paleodata_e, lwd=lns, col=col_emul)
lines(-(times2), my_tim_anom_ODP1090_paleodata_e_800, lwd=lns, col=col_emul_800)

lines(-(data_SST_anom_ODP1090_MartinezGarciaetal10_800kyr_BP_interp[,1]),data_SST_anom_ODP1090_MartinezGarciaetal10_800kyr_BP_interp[,3], lty=2, lwd=lns, col=col_proxy)
...
# Don't need to set up other constants again
...
png(file=paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Plots/SAT_DomeC_2MaBP_diffyaxis.png", sep=""),width=3000,height=850,res=300)
par(fig=c(lb,rb,(bb),(bb+ht)), mar=c(2,4.5,0.5,3), pty="m", mgp=c(2,0.5,0), las=1, lwd=lns)
plot(-(times), my_tim_anom_DomeC_paleodata_e,type="l", cex=lns, col=col_emul, xlab="", ylab="", xlim=xlimits, ylim=ylimits, xaxt="n", yaxt="n", xaxs="i", yaxs="i", axes=FALSE)
...
png(file=paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Plots/SST_anom_ODP982_2MaBP_diffyaxis.png", sep=""),width=3000,height=850,res=300)
par(fig=c(lb,rb,(bb),(bb+ht)), mar=c(2,4.5,0.5,3), pty="m", mgp=c(2,0.5,0), las=1, lwd=lns)
plot(-(times), my_tim_anom_ODP982_paleodata_e,type="l", cex=lns, col=col_emul, xlab="", ylab="", xlim=xlimits, ylim=ylimits, xaxt="n", yaxt="n", xaxs="i", yaxs="i", axes=FALSE)
...
png(file=paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Plots/SST_anom_ODP722_2MaBP_diffyaxis.png", sep=""),width=3000,height=850,res=300)
par(fig=c(lb,rb,(bb),(bb+ht)), mar=c(2,4.5,0.5,3), pty="m", mgp=c(2,0.5,0), las=1, lwd=lns)
plot(-(times), my_tim_anom_ODP722_paleodata_e,type="l", cex=lns, col=col_emul, xlab="", ylab="", xlim=xlimits, ylim=ylimits, xaxt="n", yaxt="n", xaxs="i", yaxs="i", axes=FALSE)
...
png(file=paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Plots/SST_anom_ODP1143_2MaBP_diffyaxis.png", sep=""),width=3000,height=850,res=300)
par(fig=c(lb,rb,(bb),(bb+ht)), mar=c(2,4.5,0.5,3), pty="m", mgp=c(2,0.5,0), las=1, lwd=lns)
plot(-(times), my_tim_anom_ODP1143_paleodata_e,type="l", cex=lns, col=col_emul, xlab="", ylab="", xlim=xlimits, ylim=ylimits, xaxt="n", yaxt="n", xaxs="i", yaxs="i", axes=FALSE)
...
png(file=paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Plots/SST_anom_ODP846_2MaBP_diffyaxis.png", sep=""),width=3000,height=850,res=300)
par(fig=c(lb,rb,(bb),(bb+ht)), mar=c(2,4.5,0.5,3), pty="m", mgp=c(2,0.5,0), las=1, lwd=lns)
plot(-(times), my_tim_anom_ODP846_paleodata_e,type="l", cex=lns, col=col_emul, xlab="", ylab="", xlim=xlimits, ylim=ylimits, xaxt="n", yaxt="n", xaxs="i", yaxs="i", axes=FALSE)
...
png(file=paste("C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Palaeo output 2Ma/Plots/SST_anom_ODP1090_2MaBP_diffyaxis.png", sep=""),width=3000,height=850,res=300)
par(fig=c(lb,rb,(bb),(bb+ht)), mar=c(2,4.5,0.5,3), pty="m", mgp=c(2,0.5,0), las=1, lwd=lns)
plot(-(times), my_tim_anom_ODP1090_paleodata_e,type="l", cex=lns, col=col_emul, xlab="", ylab="", xlim=xlimits, ylim=ylimits, xaxt="n", yaxt="n", xaxs="i", yaxs="i", axes=FALSE)
